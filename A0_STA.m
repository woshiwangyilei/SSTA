function [tt, D, pathnum, Q, iter] = A0_STA(startPoint, endPoint)
    ...
    while ~isequal(nowPoint, endPoint) && iter < 50
        ...
        if pr < 99
            ...
            switch choo
                case 1
                    candidate = op_expand_o(nowPoint, SampleNumber, 1); %op_expand_o(nowPoint, SampleNumber, 1); % op_expand_o([10 10 2.5], SampleNumber, 0.5);
                case 2
                    candidate = op_rotate_o(nowPoint, SampleNumber, 2); %op_rotate_o(nowPoint, SampleNumber, 5); %op_rotate(endPoint, nowPoint, SampleNumber, A0_getDist(nowPoint,endPoint));
                case 3
                    candidate = op_translate(lastPoint, nowPoint, SampleNumber, 1);
                case 4
                    candidate = op_axes(nowPoint, SampleNumber, 0.8);
            end
